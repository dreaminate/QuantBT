...
- **M4 orchestrator+新路由**（✅ **M4a 已建·core** + ✅ **M4b 已 land**）：scripted/replay 后端驱动 SSE + 真 claude stream-json 解析器 + 会话流路由。
  ...
  - **★ M4b 落地（✅ land·产品 a5af59b9）**：`claude_backend.py` 加 `parse_claude_stream_json_lines`（纯解析器·真样本 oracle=fixtures/claude_stream_json_sample.jsonl·thinking 块永不外显·result 成功须正证 subtype==success·非 dict message/坏行防御跳过·tool_use_id→name 回填）+ `ClaudeBackend`（spawn 真 claude·prompt 走 stdin·**L-C build_spawn_env 白名单**·stdin daemon 线程防死锁·stderr 临时文件·非零退出无终态→诚实 BackendError·always cleanup）+ main.py 路由 `GET /api/agent/session/stream`（镜 agent_workbench_stream:36060·owner 从 auth·preflight not-ready→诚实 error+done 不 fallback·refresh_store→跨进程写可见·backend 工厂可注入测试）。对抗 22·变异四门(os.environ.copy→master key 泄漏/thinking 外显/error→Done/refresh=None)·跨厂商 codex floor 2 轮(R1 FLOOR-HOLE 逮 3 洞→修→R2 FLOOR-HOLD)。后端 6873/0 failed。**下一 M5→已由 M5a(store L-D)+M5b(canvas_create_node 写工具) land 完成**；接 **M6 全链真 claude 冒烟+红线总断言**。
...
- **M6 全链真 claude 冒烟+红线总断言(核心)**：拆两刀。
  - **★ M6a 落地(✅ land·产品 6d0683f0)=红线总断言(CI-safe)**：`test_agent_m6_redline_total`(14)把 L-A/L-B/L-C/L-D 各层合成【跨 tier 一体】断言,对 run() 同一 builders、投毒 parent env 下验:三层枚举(registry/argv MCP 子集/mcp.json 单 server)·跨 4 tier 只 --permission-mode 变+写出 mcp.json 字节一致·canary 值扫描不入 env/mcp/argv·QUANTBT_RUNTIME_MODE 恒 agent 绝不 test/dev(否则 dev master key 现形)·词汇扫描+frozenset 封+无 register 钩·HOME 在但 master key 绝不在·解析器丢 thinking secret。**duet 逮真设计漏**:①canvas_create_node 之前不在 --allowed-tools=M5b 写工具默认 tier 不可达→修(两 canvas 工具都 pre-approve·L-D 恒钳 OFFLINE 安全)②`--allowed-tools`=免确认白名单≠工具可达集,agent 默认有 29 内建工具(Bash/WebFetch)——用户拍板 [[D-AGENT-BUILTIN-TOOLS]] 保留内建(放权·tier 门控·ambient residual 文档化),红线靠 L-A+L-C 守,**不谎称 canvas-only**。变异三门 RED-then-revert·跨厂商 codex 2 轮(R1 逮 3 test-rigor 洞→修→R2 FLOOR-HOLD)·后端 6887 passed。
  - **M6b(下切·opt-in 真 claude 冒烟)**：本地真订阅 claude 跑「读画布+加假说节点」(GOAL §0 可证伪)。gate=preflight().ready + QUANTBT_AGENT_REAL_SMOKE=1·CI 诚实 SKIP(KNOWN_RUN_GAP 非假绿)·断言真(非 mock)claude 发≥2 tool_use(read+create)·**init.mcp_servers==[quantbt-agent-canvas]**(验 --strict-mcp-config 真语义·codex B3)·落 OFFLINE 节点(ground truth)·SSE tool_start/tool_end/done·静默 mock fallback→拒·收据 JSON(本地证据·CI 不读=防假绿)。**加**:cancel/timeout/backpressure/concurrent-append(codex flag)·--add-dir owner 路径穿越守(codex B7)。
