...
- `../decisions/`（canonical 归 leader,遍历）· 由研究产出并经用户拍板的 R1–R29 / S1–S4
...
- `archive/roadmap/` · 旧路线图（被各人 `../state/` + `../board/` 取代）
- `archive/QuantBT-GOAL.original.md` · 原始 1680 行 GOAL（被蒸馏的 `../GOAL.md` 取代）

## 在研 / 创新入口
- `ideas/` · 论文研读笔记 / 原创架构 RFC / 猜想——**还没成熟到能录任务**的先落这里（不挡、不要求严格验收）。
- `active/{developer_id}/<topic>/` · 正在深挖的研究线程（镜像 `../tasks/{developer_id}/`），带工作日志。
