...
## 完成纪要（done · 第十一波 · deep-opus 线 + 中心整合 land）
**分支**：`wave11/promote-execution-honesty`（基于 origin/main 23b709d）·commit `8110f99`·中心 merge 进 center-integ。

**交付（§16 致命「未注入资产却声称已采用 / 模板基线冒充生产」在数据生产层闭合·agent synth 路径）·+73/-0 纯 additive opt-in 向后兼容**：
- `ide/promote.py`：`promote_ide_run` 加 opt-in `execution_blocks` 参数·纯透传进 run.json manifest（与 extra_metadata 同范式·不传则行为完全不变）。
- `agent/business_tools.py`：新增 `_synth_execution_blocks` 把合成回测执行诚实映射成块；`_synth_and_promote` 算 assembly_injected=False + 块落进 run.json。未注入/模板基线→`{mode:template, result_grade:production}`（组装器→evaluate_release R4+R5 标签无关硬拒）；真 live→`mode:live+live_source_ref`；mock→mock_marked。
- 新测试 `tests/test_promote_execution_honesty.py`（10 例·单元 helper + promote→组装器→门 集成两层）。

**测试**：opus scoped `10 passed in 0.98s`；定向回归 `164 passed`（ide_promote/promote_assembler/release_gate/business_tools/gate_wiring 等）。**中心全量批次 2670 passed / 13 skipped / 0 failed / 116s**（collect 2683 = 基线 2673 + 10·精确吻合·flake 未触发）+ validate PASS。

**对抗 5 条 + MUT 三态**：模板基线必被 R4+R5 抓 / 真 live 注入不误伤(无源仍 R3 拒「写 live≠免检」) / mock 必挂标识(R1) / 向后兼容(不传块既有全绿·无组装不写块) / 不静默改裁决(promote 仍成功落盘·裁决只在显式跑组装器时发生·未接端点)。MUT：未注入分支 MODE_TEMPLATE→MODE_LIVE（卡警示的冒充）→2 failed→手工复原→10 passed·绝不 git checkout。
**数学↔实现**：无新公式（分类型执行诚实数据落账·非数值）→ 不造 MathematicalArtifact。复用 mock_honesty mode/grade 单一源、run_id 身份不另造、判定全委派 evaluate_release。

**★ 行为后果（中心第十二波接端点须知）**：template 块固定 result_grade=production → R4+R5 **标签无关**触发 → 组装器接进 promote 端点后，**带组装入参但 DS-1 真注入前的 synth run 会被发版门硬拒（任何标签）**。这是 §16 正解（模板冒充所选策略不可发版）·接线时预期此拒·DS-1 真注入落地时喂 assembly_injected=True + 真 live_source_ref 即自然转 live 块过门（helper 已支持·单测覆盖）。

**诚实残余 / follow-on（执行诚实/身份未落账的其它路径·中心接端点须知）**：
- IDE 沙箱**直接** promote（非经 _synth_and_promote）时调用方未传 execution_blocks（参数已就位·IDE 端点未填·其诚实应是 live 数据+用户自著策略）——本波只接通 agent synth 路径。
- dataset_versions+checksum run.json 仍不带（KNOWN_RUN_GAPS）→ 组装器恒标缺 → 另卡。
- LLMCallRecord：synth 用 llm_client 生成码时 run.json 不记 LLM 账 → 组装器 surface 成软 gap → 中心 follow-on。
- 备注：卡里写的组装器函数名 `_execution_block_from_dict` 实际为 `_block_from_dict`·opus 按字段契约落块已端到端核对。
