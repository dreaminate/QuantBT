...
<进实现前必须全部关闭。需拍板的逐条标**规范标签** [需拍板](待) / [已决](已拍)——**只认这两个名、别用变体**(标签漂→计数连锁错)。**计数器由 `python dev/scripts/build_card_counters.py` 从标签派生写回、人别手敲**(validate 核对标签规范+计数);**全闭即 0/N 才可进实现**。非拍板的开口(留 hook / 归后续 / 实现时定)不标、不计入。>
