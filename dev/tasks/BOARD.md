...
| T-025 | 真钱执行路径审计 + 急停/kill 控件收尾 | 收口§A | todo | P1 | T-021,T-022 | 种绕门 place_order→审计必抓、kill 端点无鉴权→403、emergency 真平仓非空 log |
...
**第一波 = 簇A 脊柱收尾**（T-023/T-024/T-025，待用户过目卡 + 2 岔路点头后开跑）；第二波起按 1A（C 组合三角 + D 双时态 → B 因子轨 → E/F 交织）。
