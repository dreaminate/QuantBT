...
| Mathematical Spine 一致性门核心 + 全链贯穿第一段(DSR) | 🟡 门 + 数据模型 + DSR 真实绑定已建并验证、其余数学点贯穿待续 | `tests/test_mathematical_spine_consistency_gate.py` **28 passed** + `tests/test_spine_dsr_binding.py` **10 passed**（DSR 漂离定义→独立 oracle 对账 fail→门拒；真源码 `inspect.getsource` 指纹 staleness 抓改动）；复用 `ids.content_hash`/`ledger._ChainStore` | ✅ §6/§8 命门接活 + DSR 第一个真数学点经 binder 绑定（信任层估计器实现↔定义一致由门守）；**待续**：`verify_dsr_consistency()` 接生产 promote 路径；factor/model/signal/portfolio/execution/attribution/monitor 其余数学点逐个绑；PIT 子句未连 R28 resolver 真点查 |
