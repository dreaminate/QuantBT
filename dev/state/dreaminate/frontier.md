...
- **金融数学 kernel P0**（北极星① 数学贯穿·最大缺口）：agent epic 闭环后，转「数据→因子→模型→信号→组合→执行→回测→归因→监控」链上机构级数学的**理论先证明 → 一致性校验门**主干。分解草案见 `research/findings/dreaminate/`（金融数学 P0-P3）。实质性切片走 duet（deep-opus ‖ codex），跨厂商 floor 复审。
  ...
  - **🔨 IN-PROGRESS=P0-A #8 组合 correctness（真 ERC）——solver 已实现+数值实证，命门/测试/floor 待续（WIP·optimizers.py 工作区未提交）**：
    ...
    - **🔜 待续（next fire）**：① 新建 `app/backend/app/portfolio/spine_binding.py`（镜 risk_measures/spine_binding.py·ERC_ARTIFACT[ARTIFACT_ESTIMATOR,PROOF_BACKED]+ERC_DATA_CONTRACT[known_at=cov_estimate_known_at/effective_at=rebalance]+_ERC_IMPL_CHAIN[**仅 solver 路径**:equal_risk_contribution,_cov_to_correlation_strict,_erc_newton_solve,_erc_normalize_logdomain·**不含 inverse_volatility/oracle**]+ERC_PINNED_FINGERPRINT+dense-RC oracle+闭式 oracle+fixtures+verify_erc_consistency)② 新建 `tests/test_portfolio_erc_spine.py`（golden+对角=inv-vol+2-asset=inv-vol+dense-RC+置换等变+正尺度不变+fail-closed raises+**MUTATION:inverse_volatility 当 ERC impl→dense-RC fail→challenged**+pinned-fingerprint-matches-live+RED-then-revert)③ 扩 test_portfolio.py 加 ERC 测④ 全量后端(7019+新)⑤ 跨厂商 codex floor⑥ 四门→land。
