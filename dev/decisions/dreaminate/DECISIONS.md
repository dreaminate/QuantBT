...
- **对 W1 enforce 的影响**：artifact 信任门 enforce-默认（在飞卡 6144bd61）—— artifact RCE 是 §15 安全红线（非松紧档·该守），但按「不要管太宽」精神：中心整合时若 enforce-默认开给合法工作流加摩擦/破基线，**倾向保持 opt-in**（止血层 RestrictedUnpickler+weights_only always-on 已守直球 RCE 红线）；仅在全 producer 已接、零摩擦、全量绿时才默认开。
