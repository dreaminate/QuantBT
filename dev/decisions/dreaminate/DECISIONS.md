...
## D-ASHARE-LIVE-RATIFY · A股/全 venue live 不接实盘·治理门保留（2026-07-16 用户拍板）

> 用户 2026-07-16（AskUserQuestion「治理后可·保留未来口子」+ 指令「不接实盘、其余全做」）ratify：解 `RULES.project.md:11「A股永不实盘」` vs `GOAL.md:1787「未来治理后可 live」` 的表面矛盾。

- **决定**：real/live 交易**当前一律不接**（A股 + 全 venue，红线从 A股扩到所有真实 venue）；`GOAL.md:1787` 的「未来明确治理决策后可 live」口子**显式搁置（保留、不删）**——不是现在做，是留给未来治理门。
- **对 fork-2 agent floor 的影响**：红线 floor claim 边界 = 「当前不碰真钱/真 venue/实盘」为产品级确定 scope；floor 结构性拦「动钱/venue/live/testnet/mainnet」不变。M1-M5 canvas 写只落 OFFLINE 节点，不触 live。
- **依据/代价**：偏 RULES 侧（安全优先）；放弃「现在就上 live」，保留未来治理路径。不改 RULES.project.md/GOAL.md 原文（【开发os级别】），本条为 ratification 记录。
- **状态**：confirmed。承接：agent floor L-D CANVAS store 不变量强制 OFFLINE；A股 live 相关任务保持 deferred。

## D-AGENT-PROVIDER-CRED · 平台存 provider 凭据（双面隔离·不共 master key）（2026-07-16 用户拍板·方向）

> 用户 2026-07-16 指令「平台存 auth 后的凭据（含 apikey）·openclaw/opencode 式」——改写 fork-2 L-C「QuantBT 绝不持凭据」为**双面凭据架构**。

- **决定**：新建 **Plane A（provider 凭据面）**=Anthropic/OpenAI OAuth token + apikey（驱动模型），复用现有 keyring 机制但**独立 namespace**；**可**注入 agent env（用户自己的模型额度·低危）。**Plane B（venue 凭据面）**=binance/broker key（动真钱）保持现 `keystore.py`+`QUANTBT_MASTER_KEY`，**绝不**进 agent env。
- **不可让步不变量（安全护栏）**：Plane A/B **不共用 master key、不落同一个能被 agent-MCP 进程 import 的模块**。model apikey 进 agent env 无所谓；能解 venue keystore 的 `QUANTBT_MASTER_KEY` 进 env 才是砸穿点（L-C 红线）。
- **落地形态（据 model-switch-reference-impls 已扒 openclaw/hermes/opencode）**：OAuth 优先引用宿主 CLI 存储（`~/.claude/.credentials.json`/Keychain）；裸 apikey 才自存 keyring；落盘凭据引用 `sha256:<fp>` 脱敏；拆 session/provider token。
- **待拍工程取舍（我推荐·按放权照推·撞岔路才停）**：①存储权威=混合（OAuth 引宿主·裸 key 自存）②直连模型 API 路径=先不做（M1-M6 走 CLI-subprocess·避免重开 K3）③L-C 措辞改写=安全相关·落码前跨厂商 codex floor 复审。
- **依据/代价**：让用户 auth 一次平台托管 vs 纯依赖 CLI keychain；代价=多一个凭据面要守隔离不变量。**opendesign 参考本地零命中，待用户确指哪个仓**。
- **状态**：confirmed（方向）；实现待 M5 后按 provider-cred 切片走 + 跨厂商复审。

## D-GOVERNANCE-LIGHT · 全局轻治理原则：能力宽·治理轻·user 握方向盘（2026-07-16 用户经 agent 拍板·产品哲学）

> 用户 2026-07-16 一组决策的共同脊椎：能力面全建，治理默认 advisory 不硬阻断，唯一硬红线=不接实盘。反解跨厂商 skeptic 一路的「过度治理」张力（floor NOT SOUND 要 OS 沙箱、质量门要对抗防篡改=管太宽）。

- **决定**：所有「门」默认 **advisory/可选**（producer 出建议+代价，不阻断），user 越门自负其责——**唯一例外=硬红线**（不接实盘/不动真钱/数据泄露/A股永不实盘·D-ASHARE-LIVE-RATIFY）。运行时性能优先；研究/详情面质量优先。
- **落地**：§17 RDP require_rdp=False（advisory）；金融数学 L6「缺失即阻断」降为警告不阻断；质量门=无意损坏检测非对抗防篡改。correctness（数学↔实现一致·不假绿灯）与硬红线**不在**放宽之列——那是护栏不是治理门。
- **依据/代价**：给用户自由度 vs 强制严谨；用「摆代价+可选门」替「硬墙」。**边界**：轻治理不能松 correctness 与安全红线（护栏≠治理门）。
- **状态**：confirmed。承接：后续所有门设计默认 advisory；skeptic 复审据此校准（拦红线+correctness，不拦方法学松紧）。

## D-DUALMODEL-VERIFY-CODE · dual-model review 改验代码正确性而非重算结果（2026-07-16 用户拍板·方向·须跨厂商复审）

> 用户 2026-07-16「验代码对不对，不是验回测结果」——重定向卡 8be0e547：verifier 从「重算指标在结果上抓夸大」改为「读产出代码文件验逻辑正确性」。

- **决定**：dual_model_review verification target = **审被审方产出的代码/commit 正确性**（读代码文件·验逻辑），非重算数值。binding 把 verdict 绑到**代码/commit**（request_payload_digest 绑代码非结果）。
- **依据**：同一份 buggy 代码重算得同样错结果→只读代码抓得住「看着合理其实错」，同本仓「验机制不验输出·种坏门必抓」哲学。
- **状态**：confirmed（方向）；**安全/诚实机制改动·落码前必过跨厂商复审**（上次 NOT SOUND）。实现待排期。

## D-EPIC-PRIORITY · 两巨型 epic 排序：先收尾内嵌 agent 薄纵切再起金融数学 kernel（2026-07-16·Inference 委派自决·可翻案）

> 用户「除实盘外全做+中心自动编排」授权 parent 自决排序。两 epic：内嵌 agent（M4b→M6·近可演示成品）vs 金融数学 kernel（b4·P0-P3·基础但见效远）。

- **决定（Inference·可翻案）**：**先收尾内嵌 agent epic**（M5b→M4b→M6·近「真 claude 读画布」可演示成品·合北极星「可上线成品」），**再起金融数学 kernel P0**。
- **依据**：agent epic 快到可演示、别为开新大坑丢将成切片；math kernel 是地基但离用户可见价值远。若用户优先研究深度>agent 可用性则反转。
- **已存**：金融数学 GPT 原文（b4）已逐字落 `dev/research/findings/dreaminate/math-spine-gap-analysis-gpt-20260716.md`（2026-07-16·外部原文·未经本仓复核·引用边界=他方判断非本仓结论·状态=待评估）——防再丢完成。原文提议 6+ 条同级主干缺口 + P0-P3 建设序 + `app/backend/app/math/` typed 一等对象层；其 L0-L7 MathCapabilityMatrix 治理须按 [[D-GOVERNANCE-LIGHT]] 校准（L6 default-advisory 不硬阻断），correctness/硬红线不放宽。
- **状态**：confirmed（Inference 默认序）。承接：M5b 继续→M4b→M6→math P0（P0-P3 分解草案见 findings，仅归档不 pivot）。

## D-CANVAS-NODE-FIELDS · agent 建节点字段 = agent 全供 + UI 可编辑（边界仍归 L-D）（2026-07-16 用户拍板 Axis F「agent 全供·推荐」）

> M5b `canvas_create_node` 写工具的字段来源与可编辑性拍板。承 [[D-PERM]] 权限三态 + [[D-QRO-CANVAS]] 终态对象。

- **决策**：`canvas_create_node` 的研究内容字段（qro_type/market/universe/horizon/frequency/assumptions/known_limits/failure_modes/validation_plan）**由 agent 全供**（不留空、不 UI 必填兜底）；建成后**用户可在画布 UI 编辑**。owner 由 spawn 环境 `QB_OWNER` 注入（agent 不自选 owner）。
- **信封强制项 agent 不可动**：tool 强制 actor=AGENT / status=DRAFT / evidence=UNTESTED / runtime=OFFLINE / allowed_environment=OFFLINE / version=1 / implementation_hash=内容派生（`canvas_agent_draft:`+content_hash）。
- **边界归 L-D（store 级不变量）**：agent 供字段 + UI 编辑都**受 store 级 CANVAS 门约束**——create 只能铸 OFFLINE 草稿、edit 不能迁移 runtime/env（spine.py L-D 分支已落·M5a）。「agent 全供」不等于「agent 可越权」：越权面由 L-D 在 store 兜底，不靠 tool 自律。
- **依据/代价**：agent 全供降用户填表门槛（合北极星「小白能用」）；UI 可编辑保留人类修正口子；越权风险由 L-D 硬门收口（对抗测试 test_agent_canvas_write_redline.py 已证 create 拒非-OFFLINE / edit 不能升级）。
- **状态**：confirmed。M5b 落地中。

## D-CODEGEN-BRANCH-SESSION · §8 codegen 构建走分支/合并 + 存为 session 与 session_orchestrator 统一（2026-07-16 用户拍板 b3·方向）

> §8 编译器把画布编成可执行代码时的版本管理与会话统一拍板。承 [[D-QRO-CANVAS]] 编译器正名。

- **决策**：§8 codegen 的「构建产物」走 **git 分支/合并语义**（build→独立分支·验证过门→合并），不是就地覆盖；「存为 session」与现有 `session_orchestrator` **统一为同一研究会话模型**（不另起第二套会话状态）。
- **依据**：分支/合并给 codegen 产物可回滚、可并行、可审计的版本轴，和平台「不假绿灯·可追溯」一致；会话统一防双源漂移（session_orchestrator 已是研究会话单一源）。
- **待落地**：具体接线（分支命名/合并门/session schema 扩展）未实装——登记为方向拍板，起金融数学 kernel 之后或 agent epic 收尾时接。属工程实现非不可逆，实装时可细化。
- **状态**：confirmed（方向）。未实装。state.md 顶部刷新块登记为 pending 切片。

## D-B5-AUTO-RULINGS · b5 五项自决（echarts 懒加载已做 / 质量门=operator 裁决 / DVC 延后 / Ed25519 延后 / worktree 策略）（2026-07-16 parent 自决·可翻案）

> 用户 b5「你自己拍」授权下的五项工程取舍打包。均可逆、不越红线，按 [[D-GOVERNANCE-LIGHT]] 轻治理精神自决。

- **① echarts 懒加载**：确认采用（大 bundle 拆分方向）——前端 echarts 走动态 import 懒加载，减首屏 bundle。属已知优化，实装时接 state.md bundle 拆分切片。
- **② 数据质量门 = operator 裁决**：数据质量阈值（缺失率/异常值/停牌处理松紧）**不硬编码为不发条件**，做成 operator 可调裁决 + 摆代价（合「提供流程不强加选择」），correctness（PIT/防泄露）不在放宽之列。
- **③ DVC 延后 + ADR**：数据版本管理暂不引入 DVC（额外依赖 + 学习成本），现有 DatasetVersion→manifest→签名链已覆盖核心血统需求；延后决定写 ADR 记依据，未来数据规模上来再评估。
- **④ Ed25519 延后**：provenance receipt 签名暂用现有方案，Ed25519 迁移延后（非当前瓶颈·现签名链未证不足）。
- **⑤ worktree 策略**：agent 建代码/并行改文件用独立 git worktree 隔离（`.claude/worktrees/`），避免并行冲突污染活动 checkout——沿用现有 duet/loop worktree 隔离范式，不新建机制。
- **依据/代价**：五项均低风险可逆；打包自决省用户逐项拍板轮次（合轻治理）；任一项撞真实约束可翻案。
- **状态**：confirmed（Inference 自决·可翻案）。①已定方向待实装·②③④⑤方向拍板。

## D-AGENT-BUILTIN-TOOLS · 内嵌 agent 保留内建工具（放权·tier 门控·ambient residual 文档化）（2026-07-16 用户拍板 M6 duet 发现后）

> M6 duet（deep-opus‖codex）实证：build_agent_argv 不设 --tools，agent 默认拿全部 29 个 Claude Code 内建工具（Bash/Write/WebFetch…），tier 门控（bypassPermissions 全自动批）。承 [[D-GOVERNANCE-LIGHT]] 轻治理 + fork-2 recalibrated 放权 floor。

- **决定（用户经 AskUserQuestion 拍板）**：**保留内建工具**——不加 `--tools ""`（不收成 canvas-MCP-only）。agent 有全部内建工具，PermissionTier 门控。
- **红线怎么守（关键）**：3 硬红线（动钱/venue/A股-live）在任一选项下**结构性守住**，与内建工具无关：① **L-A**=MCP server 恰暴露 {canvas_read, canvas_create_node}，无任何 money/venue/order 工具；② **L-C**=build_spawn_env 白名单，agent env 绝无 QUANTBT_MASTER_KEY/venue secret——即便有 Bash 也够不到**我方**凭据。
- **代价（文档化 ambient residual）**：bypassPermissions 下 agent 能用 Bash/WebFetch 碰**用户自己**宿主机（HOME/网络）——**用户自己的风险**，非我方交出的密钥（fork-2 floor 已文档为放权代价）。M6 红线总断言据实断言此边界（`test_documented_residual_home_present_masterkey_absent`：HOME 在·master key 绝不在），**不谎称「agent 只碰画布」**。
- **诚实教训**：duet 逮出「argv --allowed-tools = 免确认白名单 ≠ 工具可达集」——原设计注释把它当可达集=会写出假红线断言（断言 agent canvas-only 而它其实能 shell）。跨厂商对抗 pass 在写断言【前】跑，才拦下这个自欺。
- **附带产品修（M6a·安全）**：canvas_create_node 之前不在 --allowed-tools→M5b 写工具默认 tier 不可达；现两 canvas 工具都 pre-approve（store L-D 恒钳 OFFLINE·安全）。
- **状态**：confirmed（用户拍板）。未来若某切片要收紧内建工具面，按需 `--tools <安全子集>` 逐切显式加、过红线门（非默认收紧）。

## D-MATH-DECIDER · codex/GPT-5.6-sol = 金融数学 kernel 的授权数学决策者（2026-07-16 用户拍板·授权委派）

> 用户 2026-07-16「金融数学这一块儿，你可以好好问问 codex gpt 5.6 sol xhigh，他很厉害我认可他能替我做数学上的决定」——把金融数学 kernel 的数学决定权委派给 codex。承 [[D-DUALMODEL-VERIFY-CODE]] 跨厂商复审 + [[D-EPIC-PRIORITY]] math kernel P0。

- **决定**：`app/backend/app/math/` 金融数学 kernel 的**数学口径 / 约定 / 公式 / 证明取舍**由 codex/GPT-5.6-sol（openai·xhigh）作**授权决策者**——它给的数学裁决（sign convention、estimator 口径、缩放律、coherence 要求、oracle 值、容差口径）我采纳落地。
- **委派边界（关键·不越授权）**：委派**仅限数学**。**不委派**：① 运行时新依赖仍是用户拍板项（codex 建议 numpy 声明属 hygiene 补齐已装传递依赖·非新增）② 安全不变量/RULES 红线（动钱/不可逆/A股-live/凭据）不因数学委派松动 ③ 我仍是**唯一写主仓库者**、亲手实现、亲手验 correctness（数学↔实现一致·不假绿灯）。codex 是数学决策者 + 跨厂商 verifier，不是代码提交者。
- **落地机制（FLOOR 循环）**：kernel 切片走 `codex exec`（read-only·xhigh）floor review——codex 出 **FLOOR-HOLE**（列真洞 file:line+修法）我逐条修，出 **FLOOR-HOLD**（数学 sound·假绿清除·可 land）才 land。approver≠creator 天然满足（codex verify·我 build）。
- **依据/代价**：用户明确认可 codex 数学能力并授权替其决定→省用户逐条数学拍板轮次、把机构级数学口径的裁量交给更强数学 reasoner；correctness 由我实现期黄金测试兜底（种坏门必抓·不靠 codex 自证）+ RED-then-revert 实证是第二道网。
- **状态**：confirmed（用户授权委派）。承接：所有 `app/backend/app/math/` 切片默认走此 floor 循环；数学裁决入代码 docstring 注明「codex 数学决策」来源。

## D-MATH-P0A-VARES · 金融数学 kernel P0-A 首切 = VaR/ES + Kupiec + 接 canonical 命门（2026-07-16·codex floor 循环产物）

> math kernel P0-A（[[D-EPIC-PRIORITY]] 排序后首切）落地记录。走 [[D-MATH-DECIDER]] codex floor 循环 R1→R17（codex FLOOR-HOLD）。

- **落地**：`app/backend/app/math/risk_measures/`——historical + parametric-Gaussian VaR/ES（horizon-aware·drift~h/vol~√h）、Kupiec POF backtest、`RiskMeasureSpec`（canonical content_hash spec_id）、**`spine_binding.py` 接 canonical 命门**。88 测试（math golden + spine binding）。`numpy>=1.26` 进 requirements（hygiene·已装传递依赖显式化·非新依赖）。
- **落地数值口径（codex floor 循环 R1→R17 收敛·codex 最终 FLOOR-HOLD 的裁决）**：
  - **historical VaR/ES**：`_historical_tail` 单一尾分解·rank/weights 由 **exact `Fraction`**（`Fraction(Decimal(str(c)))`·免 float(1-c)/ULP-snap/Decimal-context 误差·array-free 大 N 可测）导出·独立整数有理数 oracle（`as_integer_ratio` + 整数 ceil 除法·非共享 numpy 舍入）。ES 用**结构相干式** VaR+dot(w,L_tail−VaR)/alpha（加非负项→ES≥VaR 无经验容差）·partial weight `float(frac/n)` 单次落 float。
  - **Kupiec LR**：**Loader(2000) 二项偏差** `2·[bd0(x,np)+bd0(n−x,n(1−p))]`（两非负分量·无符号抵消·近均值 Loader 收敛级数〔缩放 v 免上溢·有限迭代+isfinite 守卫免无限循环〕·远均值 log 差·`_to_float`/`_int_ratio` 捕溢出 fail-closed·精确整数分子）。**verdict 判定**：`dev_num==0` 整数精确 null 短路(LR=0,reject=False)·稳定尾(tc≤0.5 用 cdf>tc·否则 sf<1−tc)·**数值邻域 fail-closed**（LR 落 crit 的 fuzz-validated `env=1e-11` 相对包络〔inline 进函数=进指纹〕或整个 **subnormal 比较域**〔scale<2^−1022〕或两 float 判据 mismatch → ambiguous·不静默给错 verdict）。
  - **命门 = CONNECTED**（codex 裁「首切就接」）：VAR/ES/KUPIEC artifact proof_backed + PIT data_contract + **真源码链指纹**（含 public dispatch/spec 校验/KupiecResult 契约/数值 helper/package export 源·pinned + staleness tripwire·**路径无关** content_hash 源文本）+ **独立 oracle 数值对账**（VaR:整数有理数 rank·ES:200万点网格 Riemann·Kupiec:power_divergence + (lr,p_value,reject) 三元组）→ evaluate_promotion。三门实测 promotable=proof_backed（8 子句含 pit-bound 全过）·注入漂移→对账 fail→门降 challenged。ARTIFACT_ESTIMATOR/STATISTICAL_TEST（PIT 强制）。
- **codex floor 循环 = 17 轮 approver≠creator**（R1→R16 每轮逮真 correctness/假绿洞·R17 FLOOR-HOLD）：主要防的洞类——ES 相干/5+假绿 mutation/命门真接非自证/staleness 默认+常数+契约进指纹/theory-bound+PIT taxonomy(登记待拍)/Kupiec 数值稳定性九轮（域错·精确 null 抵消·大 N near-null 抵消·float 上沿无限循环·临界 mismatch·临界同向判错·极端 tc 尺度·双下溢·subnormal-crit）。**每洞逐条修 + RED-then-revert 注入源码实证坏门必抓（36+ mutation）**；codex 独立 34650 例 120 位高精度 sweep 最终 0 错。**88 测试全绿·三指纹 pinned-match·全量后端无回归**。
- **登记 follow-on（math·加性·非本切阻塞）**：EVT tail、Christoffersen 独立性、exact-binomial 小样本覆盖、组合 correctness、measure/process typed 一等层（见 [[math-kernel-seam-matrix-20260716]] 16 trunk-lines）。
- **⚠️ 登记待用户拍板（系统级 spine_gate 硬化·非本切引入·本切不 exploit）**：codex R3 逮 **2 个 canonical 命门 pre-existing 缺口**——(A) `spine_gate.evaluate_promotion` **未强制 `binding.theory_ref==artifact.artifact_id`**（foreign-theory binding 可借壳过门·实证 ES_ARTIFACT+VaR binding 仍 promotable）；(B) **`ARTIFACT_RISK_MEASURE` 不在 `PIT_REQUIRING_TYPES`**（risk_measure artifact 可不带 PIT 过门）。**我实证试修 theory-bound → 破 8 个 section6/s6_mathchain 基线**（section6 producer 建的 artifact/binding theory_ref≠artifact_id）→ **revert**（spine_gate.py 已 byte-identical HEAD）。这俩改**动共享治理码（release_gate/section6 全委托 spine_gate）· 破现有基线 · 属方法学/OS 级门** → 登记等用户拍板做**独立系统级切片**（须同步改 section6 producer 的 theory_ref 接线）。**本 VaR/ES 切不 exploit**：verify_* 恒配对 artifact+matching binding、用 ARTIFACT_ESTIMATOR/STATISTICAL_TEST（PIT 已强制）。
- **状态**：confirmed（codex R17 FLOOR-HOLD·全量绿后 land 本切；系统级 gate 硬化独立登记待拍板）。

## D-MATH-P0A-ERC · 组合 correctness 首切 = 真 ERC + 诚实改名 + 接 canonical 命门（2026-07-16·七轮跨厂商 floor 产物）

> math kernel P0-A 第二切（承 [[D-MATH-P0A-VARES]]·[[D-EPIC-PRIORITY]] seam §8 组合 correctness 主干）。走 [[D-MATH-DECIDER]] codex 授权数学裁决 + **七轮**跨厂商 floor（approver≠creator·R1-R6 共 25 洞逐修 → R7 FLOOR-HOLD·数学主体七轮判 sound）。

- **落地**（product 单 commit·扩展不替换·8 文件）：
  - `app/backend/app/portfolio/_erc_solver.py`（**新**·ERC solver 独立模块·为命门指纹能抓 import 变更而隔离）：`equal_risk_contribution(cov,*,rc_tolerance=1e-8,max_iterations=100)` + `ERCError` + `_HARD_RC_FLOOR=1e-6` + `_cov_to_correlation_strict`/`_erc_newton_solve`/`_erc_normalize_logdomain`。
  - `optimizers.py`（M）：**诚实改名** `risk_parity→inverse_volatility`（原实为 inverse-vol `w∝1/σ`·只用对角·**仅 Σ 对角或 N=2 才恰=真 ERC**）+ 保 `risk_parity` 兼容 alias（不破基线）；从 `_erc_solver` import ERC 公开。`inverse_volatility` 留此（对角 oracle·与 solver 两条独立路径）。
  - `spine_binding.py`（**新**）：ERC_ARTIFACT（ARTIFACT_ESTIMATOR·PROOF_BACKED·ref 指 _erc_solver.py）+ ERC_DATA_CONTRACT（PIT·诚实注 raw DataFrame 无时戳）+ dense-RC 残差 oracle（+ **positivity 门**）+ 对角/2-asset 闭式 oracle + `ERC_PINNED_FINGERPRINT="0840b622b19910a7"`（含 `bridge_intact` 身份校验）+ verify_erc_consistency。
  - `__init__.py`（M·导出）、`business_tools.py`（M·equal_risk→真 ERC）、`tool_schema.py`（M·sizing 描述诚实）、`tests/test_portfolio_erc_spine.py`（**新·31 测**）、`tests/test_agent_business_tools_a4.py`（M·+4 入口测）。
- **codex 授权数学裁决（D-MATH-DECIDER·三方 duet 分歧由 codex 定）**：
  - **solver = 相关空间凸对数障碍 damped Newton**：**逐元素** R_ij=Σ_ij/(σ_iσ_j)（scale-safe·推翻「除全局 max 方差」——后者强异方差下把小 off-diag 下溢成零）；Cholesky 解方向·Armijo 保正线搜 + 近解降-E_RC 回退。推翻 CCD（Newton 有收敛证明），拒 SLSQP（非凸）。
  - **契约**：`rc_tolerance` 是 best-effort 收敛目标（仅收紧到硬地板以下）；**硬保证=返回权重 E_RC≤1e-6**（`_HARD_RC_FLOOR`）。**四终止路径**（收敛/停滞/线搜停滞/预算耗尽）对硬地板同一裁定：E_RC≤1e-6 即 return·否则 raise。
  - **独立 oracle = dense-RC 残差**（impl 诊断返 (sum(w),**positivity_flag**,N·r̂_i)、oracle 全 1·另路重算·**N≥3 相关杀伪 ERC + positivity 门杀签名象限解**）+ 对角/2-asset 闭式（vs inverse_volatility）。codex 裁：**inverse_volatility 不进指纹**（对角 oracle·两路独立）。
  - **apply_constraints blocker**→ dispatch 接线 defer follow-on·本切只 raw equal_risk_contribution。
- **命门 = CONNECTED 实证**：verify_erc_consistency(true)→**proof_backed**；MUTATION（inverse_volatility 当 ERC impl→dense-RC fail→challenged·种坏门必抓✓）；**真签名 ERC**（RC=1/N 精确含负权重）→positivity 门 fail→challenged；staleness/桥断→指纹变→exploratory。跨厂商吻合：ERC RC=(⅓,⅓,⅓)@E_RC=5e-15、inverse-vol=(3/7,3/14,5/14)；强异方差 diag(1e308) 真 E_RC=4.2e-14；200-SPD 探针 0 不收敛。
- **fail-closed**（对比旧 risk_parity 静默 σ≤0→1e-9）：非 DataFrame/非方阵(1×0 拒·真 0×0 才 {})/index≠columns/重复 label/**复数**(不丢虚部)/非有限/方差非正/`rc_tolerance` 非正/非有限/**bool** → raise（**N=1 也全验**）；非 SPD/近秩亏/超硬地板不收敛/权重下溢 → raise。后置校验**返回权重 w 本身**（u=σ⊙w·尺度消去无损·非中间 y）。metamorphic：置换等变 + 正尺度不变。
- **业务层诚实**（codex R2-R4）：equal_risk 走真 ERC·**不施 post-hoc cap**（破 RC·登记约束 ERC follow-on）·`risk_limits.max_position_enforced=false`·`covariance_source=synthetic_demo` 披露（N≥60 raise）；sizing/max_pos/dd_halt 白名单 handler 级 fail-closed（仅 key 缺失才补缺省·拒 None/bool/非有限/越界）。
- **codex floor 七轮（跨厂商·approver≠creator·数学主体 R1 判 sound：FOC/Hessian/映回/E_RC 等价/rank guard·七轮持稳）**：R1 6 洞（命门漏 positivity·equal_risk 交 inverse-vol·N=1/复数绕过·对称全局尺度·极端 SPD scale·指纹漏 import）→ R2 7 洞（含 R1 修引入的 P0 scale-first 假 ERC·synthetic 披露·risk_limits 谎报·指纹漏桥·Armijo 停滞·未知 sizing·签名测 fake-green）→ R3 4 洞（后置校验中间 y·rc_tolerance 契约·ill-cond 测 fake-green·None/bool 绕过）→ R4 4 洞（同类精修）→ R5 3 洞（rc_tolerance 契约一致性 3 面）→ R6 1 洞（预算耗尽路径）→ **R7 FLOOR-HOLD**。共 25 洞逐修 + 探针/测试实证。
- **验证**：ERC spine 31/31 + business_tools 4 入口测 + 基线 76（test_portfolio 9 + test_academic_audit_v2 28 + test_agent_business_tools_a4 39）+ 全量后端 7054 passed / 14 skipped / 0 failed（隔离 DATA_ROOT·shell timeout·真汇总行·M6b 计时 flake 属 pre-existing 无关·孤立 3/3 过）绿；compile/validate_dev/diff-check/audit 基线(61/26209663/7348e826)不变。
- **登记 follow-on（math·加性·非本切阻塞）**：(a) dispatch 接线 "erc" 进 optimize_portfolio + 非绑定约束守卫（约束绑定即 raise "constrained ERC not implemented"）(b) 真 sklearn LedoitWolf（hrp_audit.py:61·签名 cov→returns·ripple hrp_audit:134+test_academic_audit_v2:287）(c) portfolio.construct 用真 PIT 协方差替 synthetic demo。
- **⚠️ 登记待用户拍（方法学·Inference 默认可翻案·不停摆）**：① "risk_parity" 串行为(inverse-vol vs 真ERC)——选保 alias+加 equal_risk 走真 ERC ② artifact type ESTIMATOR vs PORTFOLIO_OBJECTIVE（选 ESTIMATOR·触 PIT 门零改）③ riskfolio-lib（BSD-3）作 dev-only golden·非运行时。
- **状态**：confirmed（codex R7 FLOOR-HOLD·全量绿后 land 本切；待拍板方法学项登记不阻塞）。

## D-MATH-P0A-LW · 真 Ledoit-Wolf 协方差收缩替固定-α mislabel + 接 canonical 命门（2026-07-16·五轮跨厂商 floor 产物）

> math kernel P0-A 第三切（承 [[D-MATH-P0A-ERC]] follow-on (b)「真 sklearn LedoitWolf」·同 risk_parity/ERC 一类的理论↔实现 mislabel 修）。走 [[D-MATH-DECIDER]] codex 授权数学裁决 + **五轮**跨厂商 floor（approver≠creator·R1-R4 FLOOR-HOLE 共 18 洞逐修 → R5 FLOOR-HOLD·数学核五轮判 sound 且 δ* 恒等）。

- **修的 mislabel**：`hrp_audit.ledoit_wolf_shrinkage(cov, α=0.2)` 实为**固定-α** ridge `(1-α)cov+α·tr(cov)/N·I`（docstring 自承「生产应 sklearn 自动估 α·这里固定 0.2」），被错标 Ledoit-Wolf。
- **落地**（product 单 commit `0d8d8190`·扩展不替换·5 文件）：
  - `_lw_shrinkage.py`（**新**·独立模块·为命门整模块指纹隔离）：真 `ledoit_wolf(returns)→LedoitWolfResult(cov,δ*,location)`（经 `sklearn.covariance.LedoitWolf`·BSD·requirements 已声明·prior-art）+ 诚实 `constant_shrinkage(cov,*,alpha=0.2)`（原固定-α）+ `ledoit_wolf_shrinkage` DeprecationWarning shim（出 __all__）+ `CovarianceEstimationError`。
  - `hrp_shrinkage_binding.py`（**新**·命门）：LW_ARTIFACT（ARTIFACT_ESTIMATOR·PROOF_BACKED·PIT）+ 独立手写 LW-2004 outer-product δ* oracle（另一条独立代码路径·不 import sklearn）+ 整模块指纹（`inspect.getsource` 整 `_lw_shrinkage` 模块 + `sklearn.__version__`+`numpy.__version__`+public_api+bridge_intact+solver_binding_intact）+ `LW_PINNED_FINGERPRINT="a0e2c805b1e055fd"` + verify_lw_consistency。
  - `hrp_audit.py`（M）：`optimize_hrp_safe` 用真 `ledoit_wolf`（数据驱动 δ*·非固定 0.3）；**诚实改名 `_risk_parity_weights→_inverse_volatility_weights`**（对角反波动率 w∝1/σ·不含相关性·≠真 risk parity/ERC·`fallback_used` 同改）；入口契约 fail-closed。
  - `tests/test_portfolio_lw_spine.py`（**新·31 测**）+ `tests/test_academic_audit_v2.py`（M·rewire + 改名断言）。
- **codex 授权数学裁决（D-MATH-DECIDER）**：
  - **公开 `ledoit_wolf` = classic/sklearn 口径 δ*=min(1,π̂/(T·γ̂))**·朝 scaled-identity μI 收缩·S=XᵀX/T（MLE·非 ddof=1）·μ=tr(S)/N·π̂=(1/T)Σ‖x_tx_tᵀ−S‖²_F·γ̂=‖S−μI‖²_F。**不塞 ρ̂-corrected 式**（后者 δ 差 ~0.047·非舍入·是另一 artifact·constant-correlation 目标属 LW-2003）。命门 oracle 用**同一** classic 式（否则 oracle≠impl 假 fail）。
  - impl 委托 sklearn（LW-2004 权威参考）·命门 oracle 手写 outer-product（独立路径）·两者数值到 **1e-17** 一致（codex + deep-opus 各实证）。
  - `proof_status=PROOF_BACKED` 诚实界定 = **渐近最优**（Ledoit-Wolf 2004 Thm 3.2·非有限样本 MSE 保证）。
- **命门 = CONNECTED 实证**：verify_lw_consistency(true)→**proof_backed**；MUTATION（固定-α 0.2/0.3 当 LW impl→oracle δ*≠α→dense fail→challenged·種坏门必抓✓）；staleness（current≠pinned / solver 模块级重绑 `_lw.LedoitWolf=OAS` / sklearn 升级 / 桥重绑）→exploratory。3 判别 fixture δ*=0.0517/0.0891/0.000403（距固定-α 0.2/0.3 ≥0.11·mutation 大裕度必抓）。
- **fail-closed 契约**（对比旧固定-α 静默）：非法 returns（非 2D/复数/T<2/N<1/非有限/μ=0）·scale 极端（逐元素 rescale·对称化 overflow-safe·Cholesky 因子有限）·δ*=0 秩亏 S（无条件 Cholesky）·非 SPD → raise。`optimize_hrp_safe` 入口契约：非法 returns/零方差列/重复或含首尾空白 symbol/列间动态范围过大（全局归一冲掉小尺度列）→ raise（不伪装成 inverse-vol 权重）。**全局尺度归一 xn=xc/cscale**：HRP/inverse-vol 权重与 δ* 对正尺度不变·在无量纲空间算·消 raw 单位 overflow（大尺度 np.cov→inf）与 underflow（小尺度 vol 触地板→伪等权）。scale-safe 中心化（避 `r.mean` 大均值列 sum 溢出）。
- **codex floor 五轮**（跨厂商·approver≠creator·数学主体 R1 即判 sound：公式↔论文/sklearn 一致·五轮持稳·δ* 恒等）：R1 4 洞（δ*=0 非 SPD 漏查·optimize_hrp_safe 非法输入伪权重·指纹漏模块级重绑·artifact 夸大 T<N→δ*→1）→ R2 4 洞（对称化 inf 溢出·全零短路旁路·非有限 cov 当健康·重复 symbol 权重不和 1）→ R3 5 洞（scale² 预门误拒可表示大尺度·**HRP 非全局尺度不变根因**·symbol 身份不全·fallback 错标 risk_parity·2 回归测欠力）→ R4 5 洞（`r.mean` 中心化溢出·全局归一冲掉超动态范围列·min_eigval 静默换单位·symbol 首尾空白别名·陈旧 test/doc）→ **R5 FLOOR-HOLD**。共 18 洞逐修 + **每洞 RED-then-revert 注入源码实证坏门必抓·文件复原 byte-identical**。
- **四门齐**：①最低验证=后端全量 **7084 passed/14 skip/1 failed**（真汇总行·1 failed=`test_slow_consumer_does_not_trip_idle_timeout` **pre-existing M6b 满载计时 flake·孤立非确定·`test_agent_m6b_liveness` 与 LW 无关·pre-LW 套件已在失·登记 gate③ CI-reliability**）+ LW spine 31 + affected 基线 138（academic 28 + portfolio 9 + business 39 + ERC spine 31）+ compile✓+diff-check clean+GOAL 零 diff；②validate_dev PASS；③评审=**codex 跨厂商 floor 5 轮 FLOOR-HOLD**（autoplan N/A=纯后端 math 无产品面·floor 更严·同 [[D-MATH-P0A-VARES]]/[[D-MATH-P0A-ERC]] 先例）；④data/audit 基线全程不变（61/20,339/26,209,663/`7348e826…d9c9e9`）。
- **登记 follow-on（math·加性·非本切阻塞）**：(a) 全动态范围保真 HRP（per-column log-space 协方差·替当前列间尺度差过大 fail-closed）(b) dispatch 接线真 LW 进 portfolio.construct 用真 PIT 协方差替 synthetic demo（承 ERC follow-on (c)）(c) 常数列（零方差资产/cash）纳入语义（现 LW δ*μ 正则化·未显式 fail-closed）。
- **状态**：confirmed（codex R5 FLOOR-HOLD·全量绿〔modulo pre-existing M6b flake〕后已 land 产品 `0d8d8190`；follow-on 登记不阻塞）。
