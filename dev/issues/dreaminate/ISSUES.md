...
> 闭环纪律：升任务即标 `→T-xxx`；接受为残余即标 `accepted` 并写理由；解决即标 `resolved` 并留闭合条件；**解决中踩到可复用技术坑 → 同步记一条进 `experience.md`**（ISSUES 管未决、experience 管已学,衔接别断）。
...
| I-005 | 晋级用一次性 OOS vs 运营滚动验证集 是否同切片 | 中 | T-024 Open Q | resolving |
...
### I-005 · 一次性 OOS vs 滚动验证集 [resolving]
「晋级确认用的一次性 OOS」（碰一次出裁决）与「运营 walk-forward 反复消费近段」是否同切片？**已决（2026-06-19,`DECISIONS.md` D-T024-OOS）:分两套独立切片、文档钉死各自消费口径。** **闭合条件**:T-024 落地两套切片 + 各自消费口径文档 + 对抗测试(一次性 OOS 二次消费→拒)后转 resolved。
