...
长的 read-on-demand 引用文件(`DECISIONS.md` / `GOAL.md` 这类)顶部放一句**导航 + 查法**:本文件怎么组织 + 怎么 grep 找一条,方便 agent 定位。
**铁律:只描述结构与查法,绝不枚举每条内容**——枚举=和正文双份必漂(append 一条就得回头更新顶部,十有八九忘)。要"每条都列"只能靠脚本从正文自动生成(像 `build_ledger.py`),绝不手维护。
