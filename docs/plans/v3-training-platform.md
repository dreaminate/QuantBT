# v3 · 训练台生态（ML + DL + TensorBoard）设计蓝图

> 目标（用户原话）：软件内置 xgboost / TFT 等 ML 与 DL 模型的训练生态 —— 用户与 Agent 对话输入"用哪些字段"，Agent 写代码导入**训练台**开训；**深度模型接 TensorBoard**，**非深度模型训练结束自动画图评价**，画图风格仿**回测详情页**（不止三联图，可单图）。
>
