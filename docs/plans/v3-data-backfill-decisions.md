...
> **加密数据也全部**拉；若 zip 包需要拼接就补充相关逻辑，让上层用同一套读取接口。 有问题自行决策。
...
- ✅ **完整可断点续传的拉取 + zip 拼接管线**（你要的补充逻辑和统一读取接口）
...
| 源 | 验证 | 结果 |
|---|---|---|
| Tushare（你的 token） | `build_token_pool` + `stock_basic` + `daily` 小样本（真实网络） | ✅ token 生效、**全 A股 5850 只**、daily 拉取 6000 行落地、`read_tushare` 读出 OHLCV |
| Binance Vision | 逻辑 4 单测（月+日 zip 拼接/缺口/续传/统一读取，注入 fetch） | ✅ 单测全过；⚠️ 真实下载在本机国内直连 `data.binance.vision` **超时（需代理/VPN）**——非逻辑问题 |
...
| D3 | **拼接 = 按 open_time 去重 + 排序 + 缺口检测 → 连续 parquet** | `read_klines` 返回与 REST /klines 同列连续序列，上层不用关心 zip 拼接 |
...
- `app/backend/app/data_backfill/binance.py`（月+日 zip 拼接 + 缺口检测 + 统一读取 + 全量编排 + 续传）
- `app/backend/app/data_backfill/tushare.py`（19 接口全集 + 全标的遍历 + 续传 + 统一读取 + TokenPool 接入）
...
- 可选：把 backfill 触发/进度做成 REST + 前端数据中心一个按钮（当前是 CLI；读取接口已统一）。
