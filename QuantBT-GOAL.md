...
| M9 执行 & 风控 | 🟢 v0.5 完成：ExecutionVenue 抽象 + **BacktestVenue/PaperVenue** + **BinanceSpotVenue/BinanceUMFuturesVenue (HMAC 自签 + symbol filter quantize + clientOrderId 幂等 + assert_safe_startup 拒绝 withdraw + 杠杆上限校验 + Kill Switch)** + **GenericTradingVenue (DIY YAML 交易接口)** + RiskMonitor (单笔/日内笔数/日内亏损/集中度) + SecureKeystore (keyring + Fernet AES + memory) | WS userDataStream / 三档成本预设 UI 切换 / 借贷利率自动接入 | `execution/`, `risk/`, `security/` |
...
| M14 Agent | 🟢 v0.5 完成：**LLMClient 抽象** + **DevLocalLLM (开发期我作为 LLM)** + tool_schema (13 个 OpenAPI 工具) + StrategyGoalSlotFiller (自然语言→StrategyGoal) + CodeReplicator (vnpy/backtrader/pandas/qlib → QuantBT 模板) + AgentRuntime reAct loop | 接 Anthropic/OpenAI/Qwen 真模型 / 多轮 tool 串联 | `agent/` |
| M15 前端 | 🟢 v0.5 完成：**WorkbenchPage 单页 + 5 tab**（策略工坊 / Agent 工作台 / 因子市场 / Binance 交易台 含 Kill Switch / 实验追踪）+ 顶部 nav "工坊" 入口 · TypeScript + Vite build 通过 · **RunDetailPage.tsx 0 行变更** | 拆 5 个独立页 + 真实数据图表 / ECharts | `app/frontend/src/pages/WorkbenchPage.tsx` |
