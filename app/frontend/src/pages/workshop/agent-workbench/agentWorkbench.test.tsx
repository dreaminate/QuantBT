...
describe("Agent 工作台 · 渲染骨架（T-040 / A2）", () => {
  ...
  it("台 switcher：Agent台 active（正名后非伪策略台）+ 其余 6 台皆可跳（无 soon 占位、无死链）", () => {
    const { container } = renderWithDesk(<AgentWorkbenchPage />);
    const links = Array.from(container.querySelectorAll("a")).map(
      (a) => a.textContent,
    );
    // 正名：active 台是 Agent台 —— 渲染为实心 <span>，不是 <a> 链接。
    expect(links).not.toContain("Agent台");
...
describe("§3 handoff 提交失败诚实呈现（不假绿灯）", () => {
  ...
  /** 跑完剧本铺出 handoff 卡（DS-2 默认接真后先进「看演示」demoMode 才有 mock 剧本，再点「批准本次」解锁 report + handoff）。 */
...
describe("Agent 工作台 · 措辞 / 冻结 / token 守卫", () => {
