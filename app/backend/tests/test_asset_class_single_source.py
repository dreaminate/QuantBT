"""单一身份源不变量：strategy_goal.AssetClass（窄·成本派发）⊆ instruments.spec.AssetClass（§0 全资产目录）。

`instruments/spec.py` 注释声明窄枚举与广目录「token 兼容且为其超集」（扩展不替换·不改既有窄枚举）。
...
注：这是**第八波遗留 🟡「AssetClass 双枚举收敛」的 drift-guard 部分**——把超集关系钉死；
「下游回填到广目录」是作者刻意 deferred 的更大 ripple 重构（守扩展不替换），不在本不变量 scope。
"""

from typing import get_args

from app.instruments.spec import AssetClass as BroadAssetClass
...
def test_narrow_asset_class_is_subset_of_broad_catalog() -> None:
    ...
    assert not missing, (
        ...
        "窄枚举必须是 instruments.spec.AssetClass 的子集（token 兼容超集声明 @ instruments/spec.py:41）"
    )
