...
advisory-first 不变量（本测试守门·守 LOCKED 决策 1 + RULES.project「未验证≠已验证」）：
  ...
  ③ 向后兼容：clean manifest（无 §9/§10 结构）→ 三门全 advisory ok=True·promote 与既有完全一致；
...
def test_clean_promote_backward_compat(tmp_path):
    """clean manifest（无 §9/§10 结构·不注入）→ 三门全过 advisory·promote 成功·既有键不丢（additive）。"""
    ...
    assert names == _ALL_GATE_NAMES, "registry 应把已落地三门全注册进 promote 路径"
    for v in chain["verdicts"]:
        assert v["ok"] is True, "nothing-declared → 三门全过（不误伤诚实 run）"
...
# ════════════════════════════════════════════════════════════════════════════
# ⑥ 单一注册收口：registry 注册恰三门 + ensure_default_chain 幂等（reset 安全）
# ════════════════════════════════════════════════════════════════════════════
def test_registry_registers_exactly_the_three_landed_gates():
    """registry 把已落地三门（§9/§10 成本/§10 控制面）注册进任意空链·无遗漏无多余。"""
