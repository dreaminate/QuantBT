...
def load_model(artifact_path: str | Path, *, trust: Any = None) -> Any:
    ...
    if p.suffix in (".pkl", ".joblib"):
        ...
        dossier_path = p.parent / "validation_dossier.json"
        inspection_path = p.parent / "artifact_inspection.json"
        if dossier_path.exists() or inspection_path.exists():
            _verify_governed_serialized_artifact(p)
...
def _verify_governed_serialized_artifact(path: Path) -> None:
    if path.is_symlink():
        raise ValueError(f"pickle/joblib artifact cannot be a symlink: {path}")
    dossier_path = path.parent / "validation_dossier.json"
    if not dossier_path.exists():
        raise ValueError(f"pickle/joblib artifact requires validation_dossier.json before load: {path}")
    inspection_path = path.parent / "artifact_inspection.json"
    if not inspection_path.exists():
        raise ValueError(f"pickle/joblib artifact requires artifact_inspection.json before load: {path}")
    try:
        dossier = json.loads(dossier_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"validation_dossier.json is malformed: {dossier_path}") from exc
    try:
        inspection = json.loads(inspection_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"artifact_inspection.json is malformed: {inspection_path}") from exc
    recorded_path = dossier.get("artifact_path")
    if recorded_path and Path(recorded_path).resolve() != path.resolve():
        raise ValueError(
            "validation_dossier artifact_path does not match requested artifact: "
            f"{recorded_path} != {path}"
        )
    expected_hash = dossier.get("artifact_hash")
    actual_hash = _sha256_file(path)
    if expected_hash != actual_hash:
        raise ValueError(
            "validation_dossier artifact_hash does not match requested artifact: "
            f"{expected_hash} != {actual_hash}"
        )
    inspected_path = inspection.get("artifact_path")
    if inspected_path and Path(inspected_path).resolve() != path.resolve():
        raise ValueError(
            "artifact_inspection artifact_path does not match requested artifact: "
            f"{inspected_path} != {path}"
        )
    if inspection.get("artifact_hash") != actual_hash:
        raise ValueError(
            "artifact_inspection artifact_hash does not match requested artifact: "
            f"{inspection.get('artifact_hash')} != {actual_hash}"
        )
    dossier_inspection_ref = dossier.get("artifact_inspection_ref")
    if dossier_inspection_ref and dossier_inspection_ref != inspection.get("inspection_ref"):
        raise ValueError(
            "validation_dossier artifact_inspection_ref does not match inspection record: "
            f"{dossier_inspection_ref} != {inspection.get('inspection_ref')}"
        )
    if inspection.get("accepted") is not True:
        raise ValueError("artifact_inspection rejected this artifact")
    if inspection.get("process_isolation") != "subprocess":
        raise ValueError("artifact_inspection must be produced by subprocess isolation")
    if inspection.get("inspection_mode") != "metadata_only_no_deserialize":
        raise ValueError("pickle/joblib artifact inspection must be metadata_only_no_deserialize")
    if inspection.get("deserialize_executed") is not False:
        raise ValueError("pickle/joblib artifact inspection must not deserialize the artifact")
