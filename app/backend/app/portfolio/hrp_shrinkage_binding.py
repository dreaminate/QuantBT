...
（收益窗口须实现前 PIT、无 look-ahead）。指纹含 ``sklearn.__version__`` 锁第三方行为（升级 → staleness）。
...
# 含 sklearn.__version__ 锁第三方行为（升级 → 指纹变 → staleness → 重核验）。
LW_PINNED_FINGERPRINT = "a0e2c805b1e055fd"  # 重 pin·codex floor5（scale-safe 中心化 + 动态范围 fail-closed + normalization_scale 诊断 + symbol 首尾空白拦）
...
# ── 实现链指纹（含 sklearn.__version__·public_api·bridge_intact）─────────────────
    ...
    return content_hash(
        {
            ...
            "sklearn_version": sklearn.__version__,  # 锁第三方行为·升级 → staleness → 重核验
...
def build_lw_binding(code_content_hash: str | None = None) -> TheoryImplementationBinding:
    return TheoryImplementationBinding(
        ...
        config_ref="portfolio:impl=sklearn.LedoitWolf,target=muI,assume_centered=false,cov=mle,shrinkage=analytic_delta_star",
